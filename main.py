...
if CONF_FILE.exists():
    try:
        loaded = yaml.safe_load(CONF_FILE.read_text(encoding="utf-8")) or {}
...
class ConfigPage(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        def num_entry(key, is_float=False):
            ...
            return ctk.CTkEntry(form, width=90, textvariable=self.vars[key], validate="key", validatecommand=vcmd)
        ...
        self.log = tk.StringVar(); ctk.CTkLabel(self, textvariable=self.log).grid(row=3,column=0,columnspan=2,sticky="w",padx=12,pady=(0,8))
    ...
    def save_cfg(self):
        try:
            ...
            messagebox.showerror("保存失敗", f"設定ファイルの保存中にエラーが発生しました: {e}")
    ...
    def refresh(self):
        ...
        try:
            ...
            w, h = img.size
            ratio = min(bw/w, bh/h) * self.zoom.get()
    ...
    def start_job(self): self._start_script("mosaic_generator.py", "モザイク生成中…")
    def start_rgb_gen(self): self._start_script("rgb_generator.py", "RGBリスト生成中…")

    def _start_script(self, script_name, start_msg):
        ...
        if not Path(self.vars["tile_images_folder"].get()).is_dir():
            messagebox.showerror("エラー", "タイルフォルダを選択してください")
            return
        self.save_cfg()
        self.prog.set(0)
        self.log.set(start_msg)
        self.q = queue.Queue()
        threading.Thread(target=self._worker, args=(script_name,), daemon=True).start()
