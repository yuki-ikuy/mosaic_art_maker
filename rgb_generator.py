...
def get_base_dir():
    ...
    logger.debug(f"実行ベースディレクトリ: {base}")
    return base
...
def load_config(config_path="setting/default_config.yaml"):
    ...
    # 強制的にexeと同階層のoutputディレクトリに保存
    forced_output_path = base_dir / "output" / "rgb_values.txt"

    default_config = {
        "tile_images_folder": str(base_dir / "tiles"),
        "rgb_values_file": str(forced_output_path)
    }
    ...
    if setting_path.exists():
        try:
            with open(setting_path, "r", encoding="utf-8") as f:
                ...
                logger.info(f"設定ファイルを読み込みました → {setting_path}")
        except Exception as e:
            logger.error(f"設定ファイルの読み込みに失敗しました: {e}")
    else:
        logger.warning(f"設定ファイルが見つかりません: {setting_path}")

    # 強制的に出力ファイルパスを base_dir/output/rgb_values.txt にする
    default_config["rgb_values_file"] = str(forced_output_path.resolve())

    # 絶対パス変換
    default_config["tile_images_folder"] = str((base_dir / Path(default_config["tile_images_folder"])).resolve())
    default_config["__config_path"] = str(setting_path)
...
def calculate_average_colors(image_path_str):
    ...
    try:
        ...
        logger.warning(f"画像の読み込み失敗: {image_path.name} → {e}")
        return None
...
def save_rgb_values_parallel(config=None):
    ...
    logger.info(f"{len(image_paths)} 枚の画像を並列処理します...")
    ...
    logger.info(f"RGB値を保存しました → {output_path}")
...
def main():
    try:
        ...
        logger.info("作業ディレクトリを変更しました")
        config = load_config()
        save_rgb_values_parallel(config)
    except Exception as e:
        logger.exception(f"実行中に予期しないエラーが発生しました: {e}")
